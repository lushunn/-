...
def delvar16(train_behavior_t=train_behavior_t):
    tmp=train_behavior_t['var16'][train_behavior_t['var16']>2015].index.tolist()
    for i in tmp:
        train_behavior_t.ix[i,'var16']=int(str(train_behavior_t.ix[i,'var16'])[:4])

    b=train_behavior_t['var16'].unique()
    b.sort()
    c=[i for i in range(-1,20)]
    b=dict(zip(b,c))
    train_behavior_t['var16']=train_behavior_t['var16'].replace(b)
    return train_behavior_t['var16']
    
test_behavior_A['var16']=delvar16(test_behavior_A)
train_behavior_t['var16']=delvar16(train_behavior_t)
test_behavior_B['var16']=delvar16(test_behavior_B)
#consumer_A表     
...
pred_final_B=(0.9*pred_lgm_B+0.035*pred_xgb_B+0.065*pred_gbm_B)
